    ...
    "Serializing" - {
        "Manual" - {
            ...
            "Now with autogenerated foo when encapsulating into an Array" {
                val willWorkRegardless = cborSerializer.encodeToByteArray(
                    arrayOf(
                        coseKey,
                        coseUncompressed
                    )
                )
                cborSerializer.decodeFromByteArray<Array<CoseKey>>(
                    willWorkRegardless
                ).apply {
                    ...
                    withClue("Compressed")
                    {
                        val coseKey: CoseKey =
                            CryptoPublicKey.Ec.fromJcaPublicKey(pubKey)
                                .getOrThrow()
                                .apply {
                                    this as CryptoPublicKey.Ec; useCompressedRepresentation =
                                    true
                                }.toCoseKey()
                                .getOrThrow()
