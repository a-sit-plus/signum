...
class ByteArrayView(private val a: ByteArray, private val off: Int = 0, val size: Int = a.size - off) {
    ...
    init {
        require((0 <= this.off) && (this.end <= a.size))
    }
    ...
    fun replaceWith(other: ByteArrayView) {
        require(size == other.size); other.a.copyInto(a, off, other.off, other.end)
    }

    fun xor_inplace(other: ByteArrayView) {
        require(size <= other.size); repeat(size) { this[it] = this[it] xor other[it] }
    }
}
...
fun Int.isPowerOfTwo() = this == 1 /*2^0=1*/ || ((this > 0) && ((this and (this - 1)) == 0))
