 ...
 */
val AndroidKeyStorePreferenceTests by matrixSuite (matrixConfig { execution= ExecutionMode.Sequential/*keep order for nice test report view*/ }) {
    "emu=$isEmulator" - {
            ...
            preferenceCases.forEach { strongBox ->
                "HW=${hardwareBacking.name}, SB=${strongBox.name}" - {
                    ...
                    ("device=${Build.MANUFACTURER}" +
                            " isSB=$hasStrongBox, default seclevel=$defaultSecurityLevel, ") - {
                        try {
