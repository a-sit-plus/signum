...
class JKSSigningKeyConfiguration: PlatformSigningKeyConfigurationBase<JKSSignerConfiguration>() {
    ...
    override fun validate() {
        super.validate()
        val old = signer.v
        signer { this.privateKeyPassword = this@JKSSigningKeyConfiguration.privateKeyPassword; if (old != null) old() }
    }
}
    ...
    override fun createKeyPair(config: JKSSigningKeyConfiguration): Pair<SignatureAlgorithm, KeyPair>? =
        when (val algSpec = DSL.options(config.ec, config.rsa)) {
            ...
            is SigningKeyConfiguration.RSAConfiguration -> {
                ...
                if (keySizeBytes < 64) {
                    throw UnsupportedCryptoException("The JCA has a hard lower key length limit of 64 bytes. (512 bits)")
                }
                return Pair(
                    sequenceOf(
...
internal /*actual*/ fun makePlatformSigningProvider(config: JKSProviderConfiguration): JKSProvider =
    when (val opt = DSL.options(config.ephemeral, config.withBackingObject, config.file, config.customAccessor)) {
