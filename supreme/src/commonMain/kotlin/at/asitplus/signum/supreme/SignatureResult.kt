...
sealed interface SignatureResult<out T: Signature.RawByteEncodable> {
    /** The signature succeeded. A signature is contained. */
    @JvmInline value class Success<T: Signature.RawByteEncodable>(val signature: T): SignatureResult<T>
...
val <T: Signature.RawByteEncodable> SignatureResult<T>.signature: T get() = when (this) {
...
val <T: Signature.RawByteEncodable> SignatureResult<T>.signatureOrNull: T? get() = when (this) {
    is SignatureResult.Success -> this.signature
    else -> null
}
/** Transforms this SignatureResult into a [KmmResult]. Both [Failure] and [Error] map to [KmmResult.Failure]. */
fun <T: Signature.RawByteEncodable> SignatureResult<T>.asKmmResult(): KmmResult<T> = catching { this.signature }

/** Modifies the contained [Signature], usually in order to reinterpret it as a more narrow type. */
inline fun <T: Signature.RawByteEncodable, S: Signature.RawByteEncodable> SignatureResult<T>.map(block: (T)->S) =
    when (this) {
...
/** Modifies the contained [Signature], usually in order to reinterpret it as a more narrow type. */
inline fun <T: Signature.RawByteEncodable, S: Signature.RawByteEncodable> SignatureResult<T>
...
internal inline fun signCatching(fn: ()->Signature.RawByteEncodable): SignatureResult<*> =
    catching { fn() }.fold(
        onSuccess = { SignatureResult.Success(it) },
        onFailure = { SignatureResult.FromException(it) })
