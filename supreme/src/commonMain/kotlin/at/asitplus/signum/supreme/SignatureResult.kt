...
sealed interface SignatureResult<K: KeyType, out T: CryptoSignature.RawByteEncodable<K>> {
    /** The signature succeeded. A signature is contained. */
    @JvmInline value class Success<K: KeyType,T: CryptoSignature.RawByteEncodable<K>>(val signature: T): SignatureResult<K,T>
    /** The signature failed for expected reasons. Typically, this is because the user cancelled the operation. */
    @JvmInline value class Failure(val problem: UserInitiatedCancellationReason): SignatureResult<KeyType,Nothing>
    /** The signature failed for an unexpected reason. The thrown exception is contained. */
    @JvmInline value class Error(val exception: Throwable): SignatureResult<KeyType,Nothing>
    companion object {
        ...
        fun FromException(x: Throwable): SignatureResult<KeyType,Nothing> = when (x) {
...
val <K: KeyType, T: CryptoSignature.RawByteEncodable<K>> SignatureResult<K, T>.signature: T get() = when (this) {
...
val <K: KeyType, T: CryptoSignature.RawByteEncodable<K>> SignatureResult<K, T>.signatureOrNull: T? get() = when (this) {
    is SignatureResult.Success -> this.signature
    else -> null
}
/** Transforms this SignatureResult into a [KmmResult]. Both [Failure] and [Error] map to [KmmResult.Failure]. */
fun <K: KeyType, T: CryptoSignature.RawByteEncodable<K>> SignatureResult<K, T>.asKmmResult(): KmmResult<T> = catching { this.signature }

/** Modifies the contained [CryptoSignature], usually in order to reinterpret it as a more narrow type. */
inline fun <K: KeyType, T: CryptoSignature.RawByteEncodable<K>, S: CryptoSignature.RawByteEncodable<K>> SignatureResult<K, T>.map(block: (T)->S) =
    when (this) {
...
/** Modifies the contained [CryptoSignature], usually in order to reinterpret it as a more narrow type. */
inline fun <K: KeyType, T: CryptoSignature.RawByteEncodable<K>, S: CryptoSignature.RawByteEncodable<K>> SignatureResult<K, T>
