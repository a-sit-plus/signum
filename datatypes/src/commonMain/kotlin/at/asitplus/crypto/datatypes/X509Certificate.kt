...
private fun <T> read(input: ByteArray, tag: Int, func: (ByteArray) -> T?): Pair<T, ByteArray> {
...
private fun decodeBitstring(input: ByteArray) = input.drop(1).toByteArray()

private fun CryptoPublicKey.Ec.Companion.decodeFromDer(input: ByteArray): CryptoPublicKey.Ec? = runCatching {
    var rest = input
...
private fun Instant.Companion.decodeFromDer(input: ByteArray): Instant? {
    val s = String(input)
    val isoString =
...
private fun ByteArray.readTag(tag: Byte) =
    if (this.isNotEmpty() && this[0] == tag) this.drop(1).toByteArray() else null

private fun ByteArray.readTag(tag: Int) =
