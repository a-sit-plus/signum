...
import at.asitplus.signum.indispensable.key.PrivateKey.FromPKCS8
...
import at.asitplus.signum.indispensable.key.EcPublicKey.Companion.asPublicKey
...
sealed interface PrivateKey : Asn1PemEncodable<Asn1Sequence>, Identifiable,
    Awesn1Backed<Pkcs8PrivateKeyInfo, Asn1Sequence, Pkcs8PrivateKeyInfo.Companion> {
    ...
    sealed class Impl(
        override val raw: Pkcs8PrivateKeyInfo,
    ) : PrivateKey {
        /** optional attributes relevant when PKCS#8-encoding a private key */
         ...
         * ```
         *
         * @throws Asn1StructuralException if `this` is [EcPrivateKey.WithoutPublicKey], such as decoded from minimal SEC1
         */
    ...
    companion object :
        LabelPemDecodable<Asn1Sequence, PrivateKey>(
            EB_STRINGS.GENERIC_PRIVATE_KEY_PKCS8 to checkedAsFn { source ->
                FromPKCS8.fromRaw(Pkcs8PrivateKeyInfo.decodeFromDer(source))
            },
            EB_STRINGS.RSA_PRIVATE_KEY_PKCS1 to checkedAsFn { source ->
                RsaPrivateKey.FromPKCS1.fromRaw(at.asitplus.awesn1.crypto.RsaPrivateKeyInfo.decodeFromDer(source))
            },
            EB_STRINGS.EC_PRIVATE_KEY_SEC1 to checkedAsFn { source ->
                EcPrivateKey.FromSEC1.fromRaw(at.asitplus.awesn1.crypto.EcPrivateKeyInfo.decodeFromDer(source))
            }
        ), Asn1Decodable<Asn1Sequence, PrivateKey> by FromPKCS8 {
        fun fromRaw(raw: Pkcs8PrivateKeyInfo): PrivateKey = FromPKCS8.fromRaw(raw)
        ...
        fun fromIosEncoded(keyBytes: ByteArray): KmmResult<PrivateKey.WithPublicKey<*>> = catching {
            if (keyBytes.first() == ANSIECPrefix.UNCOMPRESSED.prefixByte) EcPrivateKey .iosDecodeInternal((keyBytes))
            else RsaPrivateKey.FromPKCS1.fromRaw(at.asitplus.awesn1.crypto.RsaPrivateKeyInfo.decodeFromDer(keyBytes))
        }
    ...
    object FromPKCS8 : Asn1Decodable<Asn1Sequence, PrivateKey> {
        ...
        override fun doDecode(src: Asn1Sequence): PrivateKey {
            val raw = Pkcs8PrivateKeyInfo.decodeFromTlv(src)
            return fromRaw(raw)
        }

        @Throws(Asn1Exception::class)
        fun fromRaw(raw: Pkcs8PrivateKeyInfo): PrivateKey {
            ...
            val decoded = when (algIdentifier) {
                ...
                EcPrivateKey.oid -> {
                    if (algParams != null) {
                        ...
                        val decoded = EcPrivateKey.WithPublicKey(raw)
                        require(decoded.curve == predefinedCurve)
                        decoded
                    } else {
                        EcPrivateKey.WithoutPublicKey(raw)
                    }
                }
...
constructor(
    override val raw: Pkcs8PrivateKeyInfo,
) : PrivateKey.Impl(raw), PrivateKey.WithPublicKey<RsaPublicKey>, PrivateKey {
    ...
    override val publicKey: RsaPublicKey by lazy { RsaPublicKey(rawPrivateKey.modulus, rawPrivateKey.publicExponent) }
    ...
    override val oid = RsaPrivateKey.oid
    ...
    companion object : LabelPemDecodable<Asn1Sequence, RsaPrivateKey>(
        EB_STRINGS.GENERIC_PRIVATE_KEY_PKCS8 to checkedAsFn { source ->
            FromPKCS8.fromRaw(Pkcs8PrivateKeyInfo.decodeFromDer(source))
        },
        EB_STRINGS.RSA_PRIVATE_KEY_PKCS1 to checkedAsFn { source ->
            FromPKCS1.fromRaw(at.asitplus.awesn1.crypto.RsaPrivateKeyInfo.decodeFromDer(source))
        }
    ) {
        override fun doDecode(src: Asn1Sequence): RsaPrivateKey =
            checkedAs(PrivateKey.doDecode(src))

        val oid: ObjectIdentifier = KnownOIDs.rsaEncryption
    }

    object FromPKCS1 : Asn1Decodable<Asn1Sequence, RsaPrivateKey> {
        @Throws(Asn1Exception::class)
        override fun doDecode(src: Asn1Sequence): RsaPrivateKey = doDecode(src, null)

        /** PKCS1 decoding of an ASN.1 private key, optionally supporting attributes for later PKCS#8 encoding */
        @Throws(Asn1Exception::class)
        fun doDecode(src: Asn1Sequence, attributes: List<Asn1Element>? = null): RsaPrivateKey {
            return fromRaw(at.asitplus.awesn1.crypto.RsaPrivateKeyInfo.decodeFromTlv(src), attributes)
        }

        @Throws(Asn1Exception::class)
        fun fromRaw(raw: at.asitplus.awesn1.crypto.RsaPrivateKeyInfo, attributes: List<Asn1Element>? = null): RsaPrivateKey {
            ...
            return RsaPrivateKey(Pkcs8PrivateKeyInfo.rsa(raw, attributes))
        }
    }
 ...
 */
sealed class EcPrivateKey protected constructor(
    override val raw: Pkcs8PrivateKeyInfo,
    ...
    override val oid = EcPrivateKey.oid
    ...
    @Throws(IllegalArgumentException::class)
    constructor(
        raw: Pkcs8PrivateKeyInfo,
    ) : EcPrivateKey (raw), PrivateKey.WithPublicKey<EcPublicKey>, PrivateKey,
        ...
        override val publicKey: EcPublicKey by lazy {
            rawPrivateKey.publicKey?.let { EcPublicKey.fromAnsiX963Bytes(curve, it.rawBytes) }
        ...
        constructor(
            ...
            Pkcs8PrivateKeyInfo.ec(
                at.asitplus.awesn1.crypto.EcPrivateKeyInfo(
                    version = 1,
    ...
    class WithoutPublicKey constructor(
        ...
        /** Creates a new [EcPrivateKey .WithPublicKey] based on the passed curve. */
        fun withCurve(
            curve: ECCurve,
            ...
            return if (encodedPublicKey != null) {
                EcPrivateKey .WithPublicKey(
                    privateKey,
                    EcPublicKey.fromAnsiX963Bytes(curve, encodedPublicKey.rawBytes),
                    encodeCurve,
                    encodePublicKey,
                    attributes,
                )
    ...
    companion object : LabelPemDecodable<Asn1Sequence, EcPrivateKey >(
        EB_STRINGS.GENERIC_PRIVATE_KEY_PKCS8 to checkedAsFn { source ->
            FromPKCS8.fromRaw(Pkcs8PrivateKeyInfo.decodeFromDer(source))
        },
        EB_STRINGS.EC_PRIVATE_KEY_SEC1 to checkedAsFn { source ->
            FromSEC1.fromRaw(at.asitplus.awesn1.crypto.EcPrivateKeyInfo.decodeFromDer(source))
        }
    ) {
        val oid: ObjectIdentifier = KnownOIDs.ecPublicKey

        @Throws(Asn1Exception::class)
        override fun doDecode(src: Asn1Sequence): EcPrivateKey =
            checkedAs(PrivateKey.doDecode(src))

        internal fun iosDecodeInternal(keyBytes: ByteArray): EcPrivateKey .WithPublicKey {
            val crv = ECCurve.fromIosEncodedPrivateKeyLength(keyBytes.size)
                ?: throw IllegalArgumentException("Unknown curve in iOS raw key")
            return EcPrivateKey.WithPublicKey(
    ...
    object FromSEC1 : Asn1Decodable<Asn1Sequence, EcPrivateKey > {
        ...
        fun doDecode(
            ...
            return fromRaw(at.asitplus.awesn1.crypto.EcPrivateKeyInfo.decodeFromTlv(src), attributes)
        }

        @Throws(Asn1Exception::class)
        fun fromRaw(
            raw: at.asitplus.awesn1.crypto.EcPrivateKeyInfo,
            attributes: List<Asn1Element>? = null,
        ): EcPrivateKey {
            require(raw.version == 1) { "EC public key version must be 1" }
            val pkcs8 = Pkcs8PrivateKeyInfo.ec(raw, raw.parameters, attributes)
            return when (raw.parameters) {
                null -> EcPrivateKey.WithoutPublicKey(pkcs8)
                else -> EcPrivateKey.WithPublicKey(pkcs8)
            }
        }
    }
}

object PrivateKeyAsn1Serializer : Awesn1BackedSerializer<PrivateKey, Pkcs8PrivateKeyInfo>(
    rawSerializer = Pkcs8PrivateKeyInfo,
    encodeAs = { it.raw },
