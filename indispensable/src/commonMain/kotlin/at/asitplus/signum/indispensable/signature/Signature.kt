...
import at.asitplus.signum.indispensable.signature.Signature.NotRawByteEncodable
import at.asitplus.signum.indispensable.signature.Signature.RawByteEncodable
...
interface Signature : Awesn1Backed<SignatureValue<Asn1Element>, Asn1Element, Signature.SignatureAsn1Serializer> {

    override val raw: SignatureValue<Asn1Element>
     ...
     */
    interface RawByteEncodable : Signature {
        /**
     ...
     *
     */
    interface NotRawByteEncodable : Signature

    val humanReadableString: String get() = "${this::class.simpleName ?: "Signature"}(signature=${encodeToTlv().prettyPrint()})"


    object SignatureAsn1Serializer : TransformingSerializerTemplate<Signature, SignatureValue<Asn1Element>>(
    ...
    companion object : Asn1Decodable<Asn1Element, Signature> {
        ...
        override fun doDecode(src: Asn1Element): Signature = runRethrowing {
            when (src.tag) {
                Asn1Element.Tag.BIT_STRING -> fromRaw(
                    BitStringSignatureValue(
                        src.asPrimitive().asAsn1BitString()
                    ) as SignatureValue<Asn1Element>
                )

                Asn1Element.Tag.SEQUENCE -> fromRaw(EcdsaSignatureValue.decodeFromTlv(src.asSequence()) as SignatureValue<Asn1Element>)
...
private constructor(
    ...
    class IndefiniteLength constructor(
    ...
    ) : EcSignature(raw), NotRawByteEncodable {
        ...
        fun withScalarByteLength(l: UInt) =
            EcSignature.DefiniteLength(l, r, s)
         ...
         */
        fun guessCurve(): EcSignature.DefiniteLength {
    ...
    class DefiniteLength @Throws(IllegalArgumentException::class) constructor(
        raw: EcdsaSignatureValue,
    ) : EcSignature(raw), RawByteEncodable {
        /**
    ...
    companion object : Asn1Decodable<Asn1Element, EcSignature.IndefiniteLength> {

        fun fromRS(r: BigInteger, s: BigInteger) =
            EcSignature.IndefiniteLength(r, s)

        /** load Signature from raw byte array (r and s concatenated) */
        @Throws(IllegalArgumentException::class)
        fun fromRawBytes(input: ByteArray): EcSignature.DefiniteLength {
            require(input.size.rem(2) == 0) { "Raw signature has odd number of bytes" }
            val sz = input.size.div(2)
            return EcSignature.DefiniteLength(
                sz.toUInt(),
        ...
        @Throws(IllegalArgumentException::class)
        fun fromRawBytes(curve: ECCurve, input: ByteArray): EcSignature.DefiniteLength {
            val sz = curve.scalarLength.bytes.toInt()
            require(input.size == sz * 2)
            return fromRawBytes(input)
        }

        override fun doDecode(src: Asn1Element): EcSignature.IndefiniteLength {
...
class RsaSignature(
    ...
    companion object : Asn1Decodable<Asn1Element, RsaSignature> {
        @Throws(Asn1Exception::class)
        override fun doDecode(src: Asn1Element): RsaSignature {
            src as Asn1Primitive
            return RsaSignature(src)
        }
    }
}
