...
val PkiExtensionsDecodingTest by matrixSuite {
    ...
    "User Notice Decoding" {
        ...
        val cert = X509Certificate.decodeFromPem(certUserNoticeQualifierPem)
    ...
    "CPS Pointer Decoding" {
        ...
        val cert = X509Certificate.decodeFromPem(certCPSQualifierPem)
        val ext = cert.findExtension<CertificatePolicies>()
        val policyInfo = ext?.certificatePolicies?.get(0)
    ...
    "Alternative Names Decoding" {
        ...
        cert = shouldNotThrowAny {
            X509Certificate.decodeFromPem(sanEmailDnsIpDirnameUriPem)
        }
    ...
    // must produce valid, canonical DER (and round-trip) — the encode direction of the migrated types.

    "GeneralSubtree encodes base + IMPLICIT [0]/[1] min/max and round-trips" {
        ...
        children[1].tag.tagValue shouldBe 0uL
        children[1].tag.tagClass shouldBe TagClass.CONTEXT_SPECIFIC
        children[2].tag.tagValue shouldBe 1uL
        children[2].tag.tagClass shouldBe TagClass.CONTEXT_SPECIFIC
    ...
    "CertificatePolicyMap round-trips as a SEQUENCE of two OIDs" {
        val maps = listOf(
